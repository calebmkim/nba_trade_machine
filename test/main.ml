...
(*TEST PLAN: Essentially, we manually tested all of the methods that
  used ocaml graphics eg. we ran "make play" and then tested by seeing
  we correctly implemented the desired behavior. This included (all of
  these are in the gameplay folder): main_pages.ml, common_functions.ml,
  welcomeScreen.ml, transition.ml, state.ml, button.ml and
  assistant_gm_play.ml. Obviously, play.ml, in the state_handler folder
  ,also falls into this category as well.

  We automatically used unit testing on the methods that didn't invovle
  the ocaml graphics. Most of these involved dealing with player and
  team stats of some sort, or dealing with the Trademap module: the
  Trademap module was the data structure we used to keep the trade
  organized: it is a (string * string list) list , in which each element
  of the list is a tuple of (team_name, players_recieving).
  ...
  folder, we tested: assitant_gm_math.ml, trade_math.ml,and
  trademap.ml. *)
