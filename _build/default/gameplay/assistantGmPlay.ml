...
let show_teams_gm () =
  MainPages.start_state (size_y ());
  let msg = "Pick a team to manage" in
  let teams = JsonTranslation.team_names in
...
let make_blue_message msg =
  let () = set_color blue in
  let _ = make_button msg in
  set_color black

let make_red_button msg =
  let () = set_color red in
  let final_button = make_button msg in
  let () = set_color black in
  final_button
...
let pick_attributes team cur_attributes =
  ...
  let () = make_blue_message msg in
  let attribute_buttons = make_button_list ~max_horz attributes in
  let () = make_blue_message current_attributes_msg in
  let cur_attribute_buttons =
    make_button_list ~max_horz cur_attributes
  in
  let final_button = make_red_button finalize_msg in
  let back_button = make_red_button "Go Back" in
  let st = wait_next_event [ Button_down ] in
