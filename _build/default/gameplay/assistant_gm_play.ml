...
let handle_click_teams_gm st team_list =
  try
    let tm = find_clicked_button st team_list in
    GMAttributes tm
  with
  | NoButtonClicked -> GMTeams
...
let show_teams_gm () =
  ...
  handle_click_teams_gm st teams_list
...
let pick_attributes team cur_attributes =
  Common_functions.start_state (size_y ());
  let msg = "Please Select the " ^ team ^ "'s needs" in
  ...
  handle_click_attributes st attribute_buttons cur_attribute_buttons
    final_button cur_attributes team

let handle_click_gm_team status player_list team_name =
  try
...
let show_gm_team attriutes team_name =
  Common_functions.start_state (size_y ());
  ...
  let st = wait_next_event [ Button_down ] in
  handle_click_gm_team st player_list team_name

let handle_click_trade_recommendations () = GMTeams

let show_trade_reccomendation player attributes =
  Common_functions.start_state (size_y ());
  let att =
    "3 Point Shooting"
    (*WILL CHANGE LATER AS WE ADD MORE ATTRIBUTES*)
  in
  try
    let tmap =
      if att = "3 Point Shooting" then make_3pt_trade player
      else failwith "TODO"
    in
