...
let show_final_teams trade_map = 
  ...
  let players_moving = List.map (snd) trade_map in 
  let first_team_recieves = List.hd players_moving in 
  let _ = List.map (print_endline) first_team_recieves in 
  let button_list = List.map 
  (fun x ->  print_name x (current_x ()) (current_y ()) max_horz) team_names in 
  let st = wait_next_event [Button_down] in 
