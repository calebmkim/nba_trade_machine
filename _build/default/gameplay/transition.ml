...
let handle_click_team_transition st lst name team_list = let setting = 
(build_setting name team_list ) in
try let decision = find_clicked_button st lst in 
...
let handle_click_player_transition st button_list name trade_map= 
  ...
  let new_trade_map = 
    ...
    (List.remove_assoc (destination) trade_map) in 
