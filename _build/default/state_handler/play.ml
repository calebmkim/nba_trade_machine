...
let rec show_screen cur_state trade_map =
  ...
  | Teams ->
      let st, go_back = show_team_list (teams_in_trade trade_map) in
      if go_back then show_screen Welcome (make_trade_map [])
      else show_screen st trade_map
  ...
  | Team_transition team_name ->
      ...
      show_screen st tmap
  | FinalTeams ->
      let st, b = show_final_teams trade_map in
      if b then show_screen st (make_trade_map [])
      else show_screen st trade_map
...
and show_screen_gm cur_state attributes =
  match cur_state with
  | GMTeams ->
      let st, go_back = show_teams_gm () in
      if not go_back then show_screen_gm st attributes
      else show_screen Welcome (make_trade_map [])
  | GMRoster team ->
  ...
  | GMRecommendation player_name ->
      let st = show_trade_recommendation player_name attributes in
      show_screen_gm st []
